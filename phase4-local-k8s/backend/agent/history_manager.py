...
class HistoryManager:
    ...
    def save_user_message(self, user_id: str, conversation_id: str, message: str):
        ...
        try:
            ...
            # Log the error but don't crash the application - continue processing
            pass
    ...
    def save_ai_response(self, user_id: str, conversation_id: str, response: str):
        ...
        """
        try:
            # Ensure the conversation exists before saving the response
            self._ensure_conversation_exists(user_id, conversation_id)
    ...
    def _ensure_conversation_exists(self, user_id: str, conversation_id: str):
        """
        Ensure that a conversation exists in the database.
        If it doesn't exist, create it with the given conversation_id.

        Args:
            user_id: The ID of the user
            conversation_id: The ID of the conversation to ensure exists
        """
        from database.conversations import db_manager

        # Use the database manager to ensure the conversation exists with the specific ID
